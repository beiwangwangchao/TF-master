...
function f_getMemberPupopColumns(salesOrgType){
	...
	var memberRole = {
			display : $l("type.com.lkkhpg.dsis.common.member.dto.memberdetails.memberrole"),
			name : 'memberRole',
			isAutoComplete : false,
			align : 'center',
			render :function(data){
				for ( var i in memberRoleData) {
					if (memberRoleData[i].value == data.memberRole) {
						return memberRoleData[i].meaning;
					}
				}
			}
		};
    ...
    if(salesOrgType == 'INTEL'){
    	return [memberCode, memberName,phoneNo, email,statusDesc,memberRole];
						...
						onselected : function(data) {
							...
							if (orderType) {
								...
								case "CONPT":{
									if (linegrid) {
										linegrid.set({
											columns : o_getCreateLineColumns( false,userType)
										});
									}
									...
									if(orderType == 'STFP' || orderType == 'STFPT'){
										...
										if(staffInstore){
											for( var k=0 ;k<staffInstore.length;k++ ){
												if(marketCode ==staffInstore[k].value ){
													if('Y' ==staffInstore[k].meaning ){
														liger.get("channel").setValue('STORE');
										...
										if(userType != 'IPONT'){
											liger.get("channel").set({readonly:false});
										}
									}
								...
								case "RDEM": {
									if (linegrid) {
										linegrid.set({
											columns : o_getCreateLineColumns( true, userType)
										});
									}
	...
	var options = {
		...
		grid : {
			...
			onLoadData : function() {
				...
				this.setParm("orderType", liger.get("orderType").getValue());
...
function validatePeriod() {
	var cdate = new Date(periodDate.toString());
...
function validateDeliver(isautoship) {
	...
	if (type == "PCKUP") {
		...
		if (linegrid && isautoship) {
			linegrid.set({
				columns : o_getCreateLineColumns( isPoint,
						userType)
			});
			linegrid.reRender();
		} else {
...
function f_save(isSave, isSumbit, isNext, autoSave) {
	...
	if (isSave) {
		...
		// BEGIN #POS2SUPT-1165 xin.zhang 保存订单时判断商品是否为空
		var lineDatas1 = linegrid.getData();
		if (!lineDatas1 || lineDatas1.length == 0) {
			$.ligerDialog
		...
		for (var k = 0; k < lineDatas1.length; k++) {
			if (lineDatas1[k].itemId) {
				itemLineDatas.push(lineDatas1[k]);
			}
		}
		...
		for ( var ild in itemLineDatas) {
			if (!itemLineDatas[ild].itemId) {
				$.ligerDialog
						.error($l('type.com.lkkhpg.dsis.common.order.salesline.item_not_null'));
				return;
			}
		}
		// END #POS2SUPT-1165 xin.zhang 保存订单时判断商品是否为空
	}
	if (autoSave) {
		var orderType = liger.get("orderType").getValue();
		if (!liger.get("memberId").getValue() && orderType != 'DIRP'
	...
	//POS2SUPT-1084 2017/07/26 BEGIN
	requestData.memberId = liger.get("memberId").getValue();
//	requestData.attribute15 ="N";
//	requestData.attribute14 ="N";
	//POS2SUPT-1084 2017/07/26 END
	if (isSumbit) {
	...
	if (DsisOrderCreate.formLock) {
		submitForLock();
	}
	function submitForLock() {
        DsisOrderCreate.formLock = false;
        $.ajax({
            type : 'POST',
            async : false,
...
//							if(json.rows[0].attribute14 && json.rows[0].attribute14 == "N" &&userType == 'IPONT'){
//								$.ligerDialog.error($l('msg.info.order.special_product'));
//								return;
//							}
//							else if(json.rows[0].attribute14 && json.rows[0].attribute14 == "N" &&userType != 'IPONT'){
//								$.ligerDialog.warn($l('msg.info.order.special_product'));
//								$.ligerDialog.confirm($l("msg.info.order.special_product"), function (yes){
//									if(yes){
//										window.top.f_removeTab(tabId);
...
//								requestData.conn = false;
//								return;
//							}
//							if(json.rows[0].attribute15 && json.rows[0].attribute15 == "Y"){
//								$.ligerDialog.confirm($l("msg.info.order.first_order"), function (yes){
//									if(yes){
//										window.top.f_removeTab(tabId);
//										window.top.f_addTab('ORDER_CREATE', $l('msg.info.order.confirm'), _basePath
//												+ "/om/om_order_confirm.html?orderId=" + headerId);
...
//							}
                    ...
                    //POS2SUPT-1255  20171114 BEGIN
                    DsisOrderCreate.formLock = true;
                    //POS2SUPT-1255 20171114 END
                    return;
                }
				/*if (isNext) {
				 if (json.success) {
 ...
 * 审核订单
 */
function f_audit(isSave, isSumbit, isNext, autoSave) {
    if ($(liger.get("remark").element).hasClass("l-text-field error")) {
        return;
    }
...
//	requestData.attribute14 ="N";
   ...
   // requestData.deliveryType = "PCKUP";
    ...
    requestData.discountAmt = summary.discountAmt;
    requestData.taxAmt = $('#tax').text();
    if (requestData.orderStatus != 'CHECK') {
        if (isSave || autoSave) {
            requestData.actrualPayAmt = requestData.orderAmt;
        } else {
            requestData.actrualPayAmt = $('#actrualPayAmt').text();
    ...
    if (requestData.orderStatus == 'CHECK') {
        requestData.discountAmt = liger.get('discountAmt').getValue();
    }
    requestData.codFlag = "N";
    requestData.sourceType = "MANUL";
    requestData.salesPoints = $("#getsalespoint").text() || 0;
    ...
    function submitForLock() {
        ...
        $.ajax({
            ...
            error : function(json){
                DsisOrderCreate.formLock = true;
                //$.ligerDialog.warn('审核失败！')
            },
            success : function(json) {
                //$.ligerDialog.success('审核成功！')
				/*modified by mingqing.wei 2018.01.22 添加审核成功提示*/
                if(json.success){
                    $.ligerDialog.success("审核成功!", "提示", function(){
                        $(".l-dialog").css("top","270px");
                        var tabId = window.top.tab.getSelectedTabItemID();
                        window.top.f_removeTab(tabId);
                    });
...
//         return;
//     }
//     $.ligerDialog.confirm($l('是否审核'), function(yes) {//msg.warn.iscancel
//         if (yes) {
//             var requestData = {
//                 orderStatus:"CHECK",
//                 headerId:$("#headerId").val()
...
function f_setMember(detail) {
	...
	if(checkFlag == 'Y'){
	   if( liger.get("orderType").getValue() != 'NMDP' && liger.get("orderType").getValue() != 'STFPT'
		   &&liger.get("orderType").getValue() != 'STFP' && liger.get("orderType").getValue() != 'CONP'
			   &&liger.get("orderType").getValue() != 'CONPT' && liger.get("orderType").getValue() != 'DIRP'){
		   $.ajaxSetup({async:false});
	        $.ajax({
	             type : 'POST',
	             url  : _basePath + "/mm/member/queryMembersForJoint?memberId="+ detail.memberId,
	             success : function(json){
	            	 if(json&&json.code == "Y"){
	            		 check1 = 'Y'
	                 }
	...
	if( memberRole == "VIP" &&liger.get("orderType")&& (liger.get("orderType").getValue() == "STDP"||liger.get("orderType").getValue() == "NMDP" )){
		liger.get("orderType").setValue("VIPP");
	}
	//vip购买状态 下订单 若会员不是vip会员  订单类型切换成标准购买
	
	if( memberRole != "VIP"&&liger.get("orderType") && liger.get("orderType").getValue() == "VIPP" ){
...
function f_setLine(oldRow, newRow) {
	...
	if(!oldRow.itemSalesType || !newRow.itemId ){
		return;
	}
	...
	$.ajax({
				type : 'POST',
				async : false,
				url : _basePath + "/inv/price/queryItemPriceForOrder?itemId="
						+ newRow.itemId + "&currency="
						+ liger.get("currency").getValue() + "&uomCode="
						+ newRow.uomCode + "&salesOrgId=" + defaultSaleOrg 
						+ "&orderType="+ liger.get("orderType").getValue()
						+ "&itemSalestype="+newData.itemSalesType ,
				success : function(json) {
					if (json.success) {
						newData.itemOrderPrice = json.rows[0];
						var itemPrice = json.rows[0];
						
						newData.pv = itemPrice.pv;
						if(itemPrice.salesPriceType == 'RP'){
							summary.points + itemPrice.salesPrice;
							newData.unitRedeemPoint = itemPrice.salesPrice;
						}else{
							newData.unitOrigPrice = itemPrice.salesPrice;
						}
	...
	//#POS2SUPT-1036 2017-05-16 BEGIN
//	var checkSpecial;
//	$.ajaxSetup({async:false});
//	if(checkFlag == 'Y' || checkFlag1 == 'Y'){
//		if( liger.get("orderType").getValue() != 'NMDP' && liger.get("orderType").getValue() != 'STFPT'
//			&&liger.get("orderType").getValue() != 'STFP' && liger.get("orderType").getValue() != 'CONP'
//				&&liger.get("orderType").getValue() != 'CONPT' && liger.get("orderType").getValue() != 'DIRP'){
//	 $.ajax({
//        type : 'POST',
//        url  : _basePath + "/inv/item/checkSpecialProduct?itemId="+ newData.itemId+"&memberId="+ liger.get("memberId").getValue(),
...
//    });}}
//	 $.ajaxSetup({async:true});
//	if(check1 != 'Y' && checkSpecial == 'Y'){
//	 $.ajaxSetup({async:false});
//		if(checkFlag == 'Y' || checkFlag1 == 'Y'){
//		 $.ajax({
//	        type : 'POST',
//	        url  : _basePath + "/mm/member/checkIsExit?memberId="+ liger.get("memberId").getValue(),
//	        success : function(json){
//	          	 if(json&&json.code == "Y"){
//	          		if(userType == 'IPONT'){
//          			  $.ligerDialog.error($l('msg.info.order.special_product'));
//          			 $("#nextBtn").ligerButton('option', 'disabled', true); 
// 				   }
//          		  else{
//          		 $.ligerDialog.warn($l('msg.info.order.special_product'));
//        		 $("#nextBtn").ligerButton('option', 'disabled', false); 
//          		}}
//	          	 else{
//	          		$("#nextBtn").ligerButton('option', 'disabled', false); 
//	          	 }
//	           }
//	    });}}
//	else{$("#nextBtn").ligerButton('option', 'disabled', false);}
//		 $.ajaxSetup({async:true});
	//#POS2SUPT-1036 2017-05-16 END
}
...
function f_calculateLinePrice() {
	...
	for ( var line in lineData) {
	    ...
	    lineData[line].unitSellingPrice = lineData[line].unitOrigPrice
					- lineData[line].unitDiscountPrice;
       lineData[line].amount = Number(lineData[line].unitSellingPrice
				* lineData[line].quantity);
	...
	if (ispoint) {
		$("#TotalCurrency").text(Number(summary.points).toFixed(summary.precision));
	} else {
...
function f_queryShippingTier(address) {
	...
	$
			.post(
					_basePath + "/dm/shippingTierDtl/queryByLocation",
					param,
					function(data) {
...
function f_openConfirm() {
	...
	for ( var i in itemLineDatas) {
		if (!itemLineDatas[i].itemId) {
			...
			itemIds.push(itemLineDatas[i].itemId);
		}
	}
		//#POS2SUPT-1036 2017-05-09 BEGIN
		
	if(check1 != 'Y' && checkFlag == 'Y' && orderType != 'NMDP'&& orderType != 'DIRP'
		&& orderType != 'CONP' && orderType != 'CONPT' && orderType != 'STFP'
			&& orderType != 'STFPT'){
		$.ajaxSetup({async:false});
			  $.ajax({
	             ...
	             url  : _basePath + "/inv/item/checkIsInclude",
	             data : JSON2.stringify(itemLineDatas),
	             success : function(data){
	            	 if(data.success){
	            		 flag = data.rows[0];
	            		 }
	...
	//#POS2SUPT-1036 2017-08-16 BEGIN
		 if(checkFla != 2){
			 f_next1(itemIds,checkFla);
		 }
	//#POS2SUPT-1036 2017-08-16 END
}

function f_next1(itemIds,checkFla){
	var checkFla1;
	var checkSpecial;
	$.ajaxSetup({async:false});
	if(checkFlag == 'Y' || checkFlag1 == 'Y'){
	 ...
	 $.ajax({
        type : 'POST',
        url  : _basePath + "/inv/item/checkSpecialProduct?memberId="+ liger.get("memberId").getValue(),
        data : JSON2.stringify(itemIds),
        contentType : "application/json",
        success : function(data){
       	 if(data.success){
       		 flag = data.rows[0];
	...
	if(check1 != 'Y' && checkSpecial == 'Y'){
	 $.ajaxSetup({async:false});
		 $.ajax({
	        ...
	        url  : _basePath + "/mm/member/checkIsExit?memberId="+ liger.get("memberId").getValue(),
	        success : function(json){
	          	 if(json&&json.code == "Y"&&userType == 'IPONT'){
	          		checkFla1 = 3;
          			  return;
 				   }
          		  else if(json&&json.code == "Y"&&userType != 'IPONT'){
          			checkFla1 = 4
          			$.ligerDialog.confirm($l("msg.info.order.special_product"), function (yes){
        				if(yes){
        					f_next();
		 ...
		 $.ajaxSetup({async:true});
		if(checkFla1 == 3 && checkFla ==1){
			$.ligerDialog.error($l('msg.info.order.special_product'));
			$.ligerDialog.error($l('msg.info.order.first_order'));
			return;
		}
		else if(checkFla1 == 3 && checkFla != 1){
			$.ligerDialog.error($l('msg.info.order.special_product'));
			return;
		}
		else if(checkFla1 != 3 && checkFla == 1){
		...
		else if(checkFla1 != 4 && checkFla1 != 3){
			f_next();
		}
}
...
function initSalesOrder(isConfirm) {
	if (!isConfirm) {
        ...
        $("#auditBtn").ligerButton({
            click : function() {
                f_audit(true, false);
            },
            text : $l('审核')//sys.hand.btn.audit
		...
		var nextBtn = $("#nextBtn").ligerButton({
			click : f_openConfirm,
 ...
 * @return 列集合
 */
function o_getCreateLineColumns( isPoint, userType) {
	var unit = {
	...
	var salesTypeUrl = _basePath + '/order/salesType/queryfororder?salesOrgId='+liger.get("salesOrgId").getValue()+'&orderType='+liger.get("orderType").getValue()+'&userType='+userType;
    var itemSalesData;
    $.ajaxSettings.async = false; 
    $.getJSON(salesTypeUrl,function(data) {

		 if(data&&data.rows){
			 itemSalesData = data.rows;
			 if(itemSalesData.length>0){
				 defaultItemSalesType = itemSalesData[0].itemSalesType;
			 }else{
				 defaultItemSalesType = null;
			 }
		}
    }) 
	...
	var saleType = {
		...
		//通过后台获取获取销售类型
			saleType.editor = {
			type : 'select',
			valueField : "itemSalesType",
			textField : "itemSalesTypeName",
			cancelable : false,
			data : itemSalesData,
			onChanged : function(e) {
				//设置行
				f_setLine(e.record, e.record);
			}
		}
	//0112
...
function f_valiItemMemberRole(memberRole, isMember, isItem, line) {
	...
	if (!isMember && isItem) {//选商品
        ...
        console.log(line.purchaseMember);
        // var purchaseMemberArr = line.purchaseMember.split(";");
        //
		// if ($.inArray(memberRole, purchaseMemberArr) < 0) {//判断是否满足商品可购买角色
		// 	warnString = line.itemNumber + ";";
...
function f_updateRequired(orderStatus){
	if(orderStatus=="CHECK"){
