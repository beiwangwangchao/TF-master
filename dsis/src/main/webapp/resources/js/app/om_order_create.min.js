					...
					if (isSumbit) {
						if (json.success) {
							...
							if (data.attribute1 && data.attribute1 == "out") {
								...
								$.ligerDialog.error(data.attribute1);
								$.ligerDialog.error(
												data.attribute2
...
function f_calculateLinePrice() {
	...
	for ( var line in lineData) {
		...
		if (!summary.priceIncludeTax && summary.enableTax && lineData[line].disableTaxFlag != 'Y') {
			if(marketCode == 'CA' && lineData[line].itemNumber == '15160502'){
				lineData[line].unitSellingPrice = (lineData[line].unitOrigPrice
					    * (1 + 0.07) - lineData[line].unitDiscountPrice).toFixed(summary.precision);
				  if(lineData[line].tax){
						var laintax= Number(Number(lineData[line].tax).toFixed(summary.precision));
						summary.tax += Number(laintax * lineData[line].quantity)
					}
			}else{
			    lineData[line].unitSellingPrice = (lineData[line].unitOrigPrice
					    * (1 + summary.taxRate) - lineData[line].unitDiscountPrice).toFixed(summary.precision);
			    if(lineData[line].tax){
					var laintax= Number(Number(lineData[line].tax).toFixed(summary.precision));
					summary.tax += Number(laintax * lineData[line].quantity)
				}
	...
	if (ispoint) {
		...
		if (summary.enableTax ) {
			...
			// if(summary.priceIncludeTax){
			// summary.tax = Number((summary.currency
			// /(1+Number(summary.taxRate))*
			// summary.taxRate).toFixed(summary.precision));
			// }else{
			// summary.tax = Number((summary.currency *
			// summary.taxRate).toFixed(summary.precision));
			// }
		...
		$("#beforeTax").text(
				Number((summary.currency - summary.tax)
						.toFixed(summary.precision)));
		// if(summary.priceIncludeTax){
		// $("#beforeTax").text(Number((summary.currency -
		// summary.tax).toFixed(summary.precision)));
		// }else{
		...
		$("#TotalCurrency").text(
				Number(summary.currency.toFixed(summary.precision)));
		$("#afterTax")
				.text(Number(summary.currency.toFixed(summary.precision)));

		if (liger.get("deliveryType")
				&& liger.get("deliveryType").getValue() != "SHIPP") {
			if (Number(summary.currency + summary.adjustMents
			...
			if (Number(summary.currency + summary.shippingTier
					+ summary.adjustMents - summary.discountAmt) >= 0) {
				$("#actrualPayAmt").text(
						Number((summary.currency + summary.shippingTier
								+ summary.adjustMents - summary.discountAmt)
								.toFixed(summary.precision)));
			} else {
				Hap.showError($l("msg.error.om.actual_payment_amount_error"));
				$("#actrualPayAmt").text("0");
