...
//    }
    if (!window.Hap) {
        ...
        // 显示成功提示窗口
        Hap.showSuccess = function(message, callback, options) {
            if (typeof (message) == "function" || arguments.length == 0) {
                callback = message;
                message = $l('sys.hand.tip.success');
            }
            $.ligerDialog.success(message, $l('sys.hand.tip.info'), callback, options);
        };
        // 显示失败提示窗口
        Hap.showError = function(message, callback, options) {
            if (typeof (message) == "function" || arguments.length == 0) {
                callback = message;
                message = $l('sys.hand.tip.failure');
            }
            $.ligerDialog.error(message, $l('sys.hand.tip.info'), callback, options);
        };
        // 覆盖页面grid的loading效果
        Hap.overrideGridLoading = function() {
            $.extend($.ligerDefaults.Grid, {
                onloading : function() {
                    Hap.showLoading($l('sys.hand.tip.loading'));
                },
                onloaded : function() {
                    Hap.hideLoading();
                }
            ...
            $.ajax({
                ...
                success : function(json) {
                    if(json&&json.success){
                        ...
                        //统一ajax异常处理
                        //$.ligerDialog.alert(json.message || 'Error', options.infoTip || $l('sys.hand.tip.info'), 'error');
                    }
                },
                error : function() {
        ...
        Hap.defaultSuccessHandler = function(opt){
            ...
            if (json && json.success) {
                ...
                if(options.success){
                    ...
                    var manager = $.ligerDialog.open({ cls: 'success', type: 'success', content: '<div style="padding:4px">'+(options.successTip || $l('sys.hand.tip.success'))+'</div>', allowClose: false });
                    setTimeout(function (){
                        manager.close();
                    }, 1000);
                }
        ...
        Hap.showInvalid = function(form) {
            ...
            if (!validator)
                return;
            var message = '<div class="invalid">' + validator.errorList.length + $l('sys.hand.tip.invalid_field') + '</div>';
        ...
        Hap.gridQuery = function(options) {
            ...
            if (!form || !grid)
                $.ligerDialog.alert('"grid" or "form" not found in options!', $l('sys.hand.tip.info'), 'error');
            if(!(form instanceof $.ligerui.controls.Form)){
                form = $.ligerui.get(form);
            }
        ...
        if (json && json.success === false) {
            if(json.code == 'session_expired') {
                if(top.sessionExpiredLogin){
                    ...
                    alert($l('sys.hand.tip.session_expired'));
